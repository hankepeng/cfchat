...
export function showImageModal(src) {
	const modal = createElement('div', {
		...
		<div class="img-modal-content">
			<div class="img-modal-content-overflow">
				<img src="${src}" class="img-modal-img"/>
			</div>
			<span class="img-modal-close">&times;</span>
			<div class="img-modal-hint">${t('ui.zoom_hint', 'Scroll to zoom / Double-click to reset')}</div>
		</div>
	`);
	...
	const closeBtn = $('.img-modal-close', modal);
	const img = $('img', modal);
	...
	on(modal, 'click', (e) => {
		if (e.target === modal || e.target.closest('.img-modal-content') === null) {
			closeModal();
		}
	});
	...
	on(img, 'touchmove', function(ev) {
		ev.preventDefault();
		if (ev.touches.length === 1 && isDragging && scale > 1) {
			// Drag
			offsetX += ev.touches[0].clientX - lastX;
			offsetY += ev.touches[0].clientY - lastY;
			lastX = ev.touches[0].clientX;
			lastY = ev.touches[0].clientY;
			updateTransform();
		} else if (ev.touches.length === 2 && initialDistance > 0) {
			// Pinch zoom
			const touch1 = ev.touches[0];
			const touch2 = ev.touches[1];
