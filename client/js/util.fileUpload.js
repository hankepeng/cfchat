...
async function handleSendFiles() {
	...
	try {
		...
		// Send files through callback with type indicator
		if (imageFiles.length > 0 && nonImageFiles.length === 0) {
			// All images - send as image message
			await onSendCallback(imageFiles, { isImages: true });
		} else if (nonImageFiles.length > 0 && imageFiles.length === 0) {
			// All non-images - send as file message
			await onSendCallback(nonImageFiles, { isImages: false });
		} else {
			// Mixed - send separately
