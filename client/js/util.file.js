...
export function setupFileSend({
	...
	if (attachBtn) {
		...
		attachBtn.addEventListener('click', (e) => {
			...
			showFileUploadModal(async (files, options = {}) => {
				...
				// Check if sending images
				if (options.isImages) {
					await handleImagesUpload(files, (msg) => {
						onSend({ ...msg, userName });
					});
				} else {
...
// Handle images upload - send images directly without compression
// 处理图片上传 - 直接发送图片无需压缩
async function handleImagesUpload(files, onSend) {
	...
	try {
		...
		// Send images as a single message
		onSend({
			type: 'image',
			data: {
		...
		if (window.addSystemMsg) {
			window.addSystemMsg(`Failed to send images: ${error.message}`);
		}
	}
}
	...
	return new Promise((resolve) => {
		...
		reader.onload = (e) => {
			...
			img.onload = () => {
				...
				// Export as webp with good quality
